...
<head>
<?php $this->display('inc_skin.php', 0 , '合买记录'); ?>
...
$(function(){
	...
	$('.bottompage a[href]').live('click', function(){
            ...
            if(href.indexOf('searchCombineRecord')>0){
                $('.biao-cont').load($(this).attr('href'));
            }else{
                $('.pagemain').load($(this).attr('href'));
            }
...
function combineSearch(err, data){
    if(err){
		alert(err);
	}else{
		$('.biao-cont').html(data);
...
function deleteBet(err, code){
	...
	if(err){
		alert(err);
            ...
            $('.pagemain').load('/index.php/record/search');
//		recodeRefresh();
	}
}
</script>
...
<div style="width:880px; height:42px; border-bottom:11px solid #ff9f08;">
	<div style="width:100%; height:30px; margin-top:10px;">
        ...
        <div class="dh_68_1" style="width:700px; float:left; height:30px;">
        	<ul>
            	<li class="fontback"><a href="/index.php/record/search"><font style="color:#890e0e;">合买记录</font></a></li>
            </ul>
        </div>
    </div>
</div>
<div class="content3 wjcont">
 <div class="body">
 <div class="youxi1">
    <form action="/index.php/record/hemai/<?=$this->userType?>" dataType="html" call="combineSearch" target="ajax">
  		<h2>
       ...
       <select name="mode" class="text5">
            ...
            <option value="1.000">元</option>
            <option value="0.100">角</option>
            <option value="0.010">分</option>
